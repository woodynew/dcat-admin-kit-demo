...
class DemoRecordController extends AdminController
{
    ...
    {
        return $this->page($content, '共享记录',
            '所有访客共享这组记录。新增、编辑、复制和快捷状态更新都实际保存，每小时统一重置。', $this->grid());
    }

    public function create(Content $content)
    {
        $copy = request()->query('copy');
        abort_if($copy !== null && (! is_scalar($copy) || ! ctype_digit((string) $copy)), 422, '复制来源无效。');
        $source = $copy ? DemoRecord::query()->findOrFail($copy) : null;

        return $this->page($content, $source ? '复制为新记录' : '创建记录',
            $source ? '已预填来源记录的内容。新编码已生成，余额从 0 开始；点击提交后才会创建。'
                : '填写内容后使用右上方「提交」或表单底部的原生提交按钮。',
            DemoExamples::recordForm(true, DemoExamples::copyDefaults($source)));
    }

    public function edit($id, Content $content)
    {
        return $this->page($content, '编辑记录 #'.(int) $id,
            '完整保存后，下方日志会显示 AdminFormUtil = true；列表内快捷状态更新为 false。', $this->form()->edit($id));
    }

    public function show($id, Content $content)
    {
        return $this->page($content, '记录详情 #'.(int) $id,
            '当前共享记录的真实数据。返回编辑页可以体验复制、顶部返回和原生提交。', $this->detail($id));
    }
    ...
    {
        return $content->title($title)->description('可验证的表单交互')
